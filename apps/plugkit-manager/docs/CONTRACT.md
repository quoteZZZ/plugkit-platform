# PlugKit 插件平台 — 识别与对接契约（CONTRACT）

> 本文件定义 **plugkit-manager（插件平台）** 与 **被它管理的系列插件** 之间的识别与通信约定。
...
- **连接许可**：所有 PlugKit 系列插件（含本平台）在 manifest **显式声明** `externally_connectable: { ids: ["*"] }`（允许所有扩展连接，与 Chrome 未声明时的默认行为一致，但更明确、不受浏览器版本行为差异影响）；信封上的 `__plugkit__` 标识用于隔离，非本系列消息会被忽略。

> **`externally_connectable` 安全考量**
> - 本系列插件以「加载已解压的扩展程序」方式使用，extension ID 由加载路径决定且不稳定；用 `ids: ["*"]` 通配显式放行，避免 ID 变化导致跨插件日志失效。
> - 权限面兜底：`__plugkit__` 信封 + 通道名白名单做第一道隔离；被管理插件不返回业务敏感数据，仅回日志（`LogEntry` 无 Cookie/Token）。
> - 若将来上架商店（store 发布时 ID 固定），建议将各插件收窄为 `externally_connectable: { ids: ["<manager的固定ID>"] }`，并将本说明同步删除。
